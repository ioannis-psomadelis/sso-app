...
- `04-keycloak-federation.excalidraw` - Keycloak federation
...
│   │   │   │   ├── federated.ts   # Keycloak federation
...
### 1.3 Federation Endpoint (Keycloak)
...
const KEYCLOAK_CONFIG = {
  ...
  // URLs constructed from baseUrl
  authorizeUrl: `${baseUrl}/protocol/openid-connect/auth`,
...
interface FederationState {
  ...
  scope: string;
  pkceVerifier: string;        // NEW PKCE for Keycloak
}
```
...
```
┌─────────────────────────────────────────────────────────────┐
│           /auth/federated/keycloak/start                    │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  1. Validate client and redirect_uri                        │
│                                                              │
│  2. Generate NEW PKCE for IdP → Keycloak                    │
...
│     /realms/sso-demo/protocol/openid-connect/auth           │
...
┌─────────────────────────────────────────────────────────────┐
│           /auth/federated/keycloak/callback                 │
├─────────────────────────────────────────────────────────────┤
│                                                              │
│  1. Read federation state from cookie                       │
│                                                              │
│  2. Exchange Keycloak code for tokens                       │
│     POST /realms/sso-demo/protocol/openid-connect/token     │
│     → code + code_verifier (from federation state)          │
│                                                              │
│  3. Fetch user info from Keycloak                           │
│     GET /realms/sso-demo/protocol/openid-connect/userinfo   │
...
│       (userId, provider='keycloak', providerSub)            │
│                                                              │
...
interface AuthContextType {
  ...
  loginWithKeycloak: () => void;  // Federation
  logout: () => Promise<void>;
...
```bash
# Keycloak Federation
KEYCLOAK_CLIENT_ID=sso-demo-client
KEYCLOAK_BASE_URL=https://auth.university.edu/realms/your-realm
```
...
- **Federation/Broker Pattern** για εξωτερικούς providers
