...
export function Landing() {
  ...
  return (
    <div className="min-h-[calc(100vh-3.5rem)] relative overflow-hidden">
      ...
      <div className="absolute inset-0 bg-gradient-to-br from-violet-500/5 via-purple-500/5 to-violet-600/10 dark:from-violet-500/10 dark:via-purple-500/10 dark:to-violet-600/20" />

      {/* Animated gradient orbs */}
      <div className="absolute top-20 -left-32 w-96 h-96 bg-gradient-to-br from-violet-500/20 to-purple-500/20 rounded-full blur-3xl animate-pulse" />
      <div className="absolute bottom-20 -right-32 w-96 h-96 bg-gradient-to-br from-purple-500/20 to-violet-500/20 rounded-full blur-3xl animate-pulse [animation-delay:1s]" />
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-gradient-to-br from-violet-500/10 to-purple-500/10 rounded-full blur-3xl" />

      {/* Floating geometric shapes */}
      <div className="absolute top-32 right-20 w-20 h-20 border-2 border-violet-500/30 rounded-2xl rotate-12 animate-bounce [animation-duration:3s]" />
      <div className="absolute bottom-40 left-20 w-16 h-16 bg-gradient-to-br from-violet-500/30 to-purple-500/30 rounded-full animate-bounce [animation-duration:4s] [animation-delay:0.5s]" />
      <div className="absolute top-60 left-1/4 w-8 h-8 bg-gradient-to-br from-purple-500/40 to-violet-500/40 rotate-45 animate-bounce [animation-duration:2.5s] [animation-delay:1s]" />
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[calc(100vh-3.5rem)] px-4 py-12">
        ...
        <div className="text-center max-w-2xl mx-auto mb-12 animate-in fade-in slide-in-from-bottom-4 duration-700">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center items-center mb-8">
            <Button
              ...
              size="lg"
              className="w-full sm:w-auto bg-gradient-to-r from-violet-500 to-purple-600 hover:from-violet-600 hover:to-purple-700 shadow-xl shadow-violet-500/25 hover:shadow-violet-500/40 transition-all duration-300 hover:scale-105 px-8 text-white border-0"
            >
        ...
        <div className="grid sm:grid-cols-3 gap-4 max-w-3xl mx-auto w-full animate-in fade-in slide-in-from-bottom-8 duration-700 [animation-delay:200ms]">
          {features.map((feature, i) => (
            <div
              key={feature.title}
              className="group relative p-6 rounded-2xl bg-card backdrop-blur-sm border border-border hover:border-violet-500/30 transition-all duration-300 hover:shadow-xl hover:shadow-violet-500/10 hover:-translate-y-1"
              style={{ animationDelay: `${(i + 1) * 100}ms` }}
            >
              <div className="size-10 rounded-xl bg-gradient-to-br from-violet-500 to-purple-600 flex items-center justify-center mb-4 shadow-lg shadow-violet-500/25 group-hover:scale-110 transition-transform duration-300">
